...
usage () {
	...
	set +o xtrace
	echo "${script_name} - Runs a TDD builder container.  If no command is provided, runs an interactive container with the current directory as the container's working directory." >&2
...
echo "PS1='${ansi_red}\h@\${P_HOST}:${ansi_reset}\w\$ '" >> "${tmp_dir}/.bashrc"
