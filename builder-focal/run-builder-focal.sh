...
usage () {
	...
	set +o xtrace
	{
		echo "${script_name} - Runs a builder-focal container."
...
if [ "${TDD_BUILDER_UBUNTU_FOCAL:-}" ]; then
	echo "${script_name}: ERROR: Already in builder-focal." >&2
	exit 1
fi

docker_args=''
container_name=${container_name:-"builder-focal"}
docker_tag=${docker_tag:-"glevand/builder-focal:latest"}
as_root=''
usage=''
verbose=''
debug=''
