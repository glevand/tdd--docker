...
usage () {
	...
	set +o xtrace
	{
		echo "${script_name} - Runs a builder-xenial container."
...
if [ "${TDD_BUILDER_UBUNTU_XENIAL:-}" ]; then
	echo "${script_name}: ERROR: Already in builder-xenial." >&2
	exit 1
fi

docker_args=''
container_name=${container_name:-"builder-xenial"}
docker_tag=${docker_tag:-"glevand/builder-xenial:latest"}
as_root=''
usage=''
verbose=''
debug=''
