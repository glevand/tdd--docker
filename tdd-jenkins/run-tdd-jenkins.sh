...
usage() {
	...
	{
		echo "${script_name} - Runs a TDD Jenkins container."
...
container_name='tdd-jenkins'
docker_args=''
docker_tag='glevand/tdd-jenkins:latest'
as_root=''
usage=''
verbose=''
debug=''
...
docker_extra_args='--mount type=volume,source=jenkins-home,dst=/var/jenkins_home --mount type=bind,source=/run/docker.sock,dst=/run/docker.sock'
